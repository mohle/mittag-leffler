/* Filename: mitlef_v1.c
   Version:  Standalone ANSI C Version 1.0
   Date:     May 22, 2022
   ...
   Please compile this file using the command "gcc mitlef_v1.c -o mitlef.exe".
   ...
   For an analog program written in C++11 see "ml_v1.cpp". */
...
Then X := -b*(log(U_1)+...+log(U_c)) is Erlang distributed with parameter c */
   double s;
   int i;
   s=0.0; for(i=1;i<=c;i++) s+=log(uniform_rand());
   return -b*s;
} /* end erlang_rand */
